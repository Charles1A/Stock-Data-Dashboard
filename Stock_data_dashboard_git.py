...
if analysis == 'Multi Percent change' and len(ticker_list) >= 2 and len(ticker_list) <= 4 and index != '-':
    ...
    st.write(f"**Below, */Lookback period*/**: the number of closed U.S. market trading days counted back from the most-recent, {d1}")
    st.write(f"Example: **5d** includes data from the 5 closed trading days between {d5} and {d1}")
