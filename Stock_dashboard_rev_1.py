...
# weeks_ago_52 = datetime.now() - timedelta(weeks=52) # computes 52 weeks ago
# api_start_date = weeks_ago_52.strftime('%Y-%m-%d') # reformats start date for yf.download API
api_end_date = date.today().strftime('%Y-%m-%d') # reformats end date for yf.download API
...
def pct_change_func(stock_df): # Takes the dataframe returned by stock_data() as an argument
    ...
    pct_chg_df = (stock_df.iloc[-1, :] - stock_df.iloc[-5, :])/stock_df.iloc[-5, :] # Computes percent change
    ...
    pct_chg_df['10d'] = (stock_df.iloc[-1, :] - stock_df.iloc[-10, :])/stock_df.iloc[-10, :] # Computes percent change

    pct_chg_df['15d'] = (stock_df.iloc[-1, :] - stock_df.iloc[-15, :])/stock_df.iloc[-15, :] # Computes percent change

    pct_chg_df['30d'] = (stock_df.iloc[-1, :] - stock_df.iloc[-30, :])/stock_df.iloc[-30, :] # Computes percent change

    pct_chg_df['90d'] = (stock_df.iloc[-1, :] - stock_df.iloc[-90, :])/stock_df.iloc[-90, :] # Computes percent change

    pct_chg_df['1yr'] = (stock_df.iloc[-1, :] - stock_df.iloc[1, :])/stock_df.iloc[1, :] # Computes percent change

    pct_chg_df = pct_chg_df.T # transpose the dataframe to re-orient the tickers
...
if analysis == 'Multi Correlation' and len(ticker_list) >= 2 and len(ticker_list) <= 4 and index != '-':
    ...
    with col1:
        ...
        lookback_days = st.slider(label = 'Use the slider to select the number of closed trading days to include in correlation computation', 
        min_value=10, 
        max_value=170, 
        value=90, 
        step=10)
